function wui.updateChannelDisplay(eventName, ...)
  ...
  if eventName == "gmcp.Comm.Channel" then
   ...
   -- Send chats to the right tab and with the right name and coloring
    ...
    local cc = wui.channelColor[gmcp.Comm.Channel.channel].channelColor or "<reset>"
    local sender = gmcp.Comm.Channel.sender or "unknown"
    local tabName = {wui.channelColor[gmcp.Comm.Channel.channel].channelName,"All"}
    ...
    for _,tab in ipairs(tabName) do
      if tab == "All" then
        wui.channelDisplay:cecho(tab, time.."<reset>("..cc..channel:title().."<reset>) "..senderColor..sender.."<reset>: "..gmcp.Comm.Channel.text)
      else
        wui.channelDisplay:cecho(tab, time..senderColor..sender..cc.."<reset>: "..gmcp.Comm.Channel.text)
      end
    end
  end
end